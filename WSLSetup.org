...
#+begin_src sh :tangle /sudo:root@localhost:/etc/rc.arch.local (identity #o744)
  #!/bin/bash
  service cron start
  service ssh start
#+end_src
...
*Note:* If you tangled the above code block instead of just copying it into  [[/sudo:root@localhost:/etc/sudoers][/etc/sudoers]], you'll need to uncomment the =#includedir /etc/sudoers.d= at the end of that file.
